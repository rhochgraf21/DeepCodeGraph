...
class D2Diagram(GraphGenerator):
    ...
    def generate(self, repository_data: Dict[str, Any]) -> str:
        ...
        logger.info(f"Token count for D2Diagram: {tokens}")
        ...
        if tokens > self.token_limit * self.fallback_threshold:
            ...
            try:
                ...
                logger.warning("No files found in repository_data for D2Diagram.")
                prompt_input = "{}"
        else:
    ...
    def save(self, graph: str, output_path: str) -> None:
        ...
        if Path(output_path).is_dir():
            ...
            output_filename = sanitize_filename(
                f"dcg-{self.__class__.__name__}-{timestamp}.d2" # Ensure .d2 extension
            )
        ...
        try:
            with open(output_file, "w+", encoding='utf-8') as f:
                f.write(graph)
            print(f"D2 diagram saved to {output_file}")
        except IOError as e:
            logger.error(f"Failed to save D2 diagram to {output_file}: {e}")
            raise
